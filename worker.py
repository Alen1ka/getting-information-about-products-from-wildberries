...
def save_answer_kafka(response, page_number):
    ...
    # Запуск callback функции для получения отчета о доставке из предыдущих вызовов produce()
    p.poll(0)

    # Асинхронное добавление сообщения в очередь сообщений в топик (отправка брокеру)
